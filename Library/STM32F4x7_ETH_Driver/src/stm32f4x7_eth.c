...
//static uint32_t gTcpDataMark; //0:没有数据 1：已经收到数据
static uint32_t gLwipDataTime; //lwip 超时标志 0：没有超时 1：超时
//static uint32_t gEthResetMark; //网络重启数据标志 0；没有数据 1 有数据
...
* @Arguments   : nCount: specifies the delay time length.
* @Returns     : None
**********************************************************************************************************/
...
void ETH_CntInit(void)
{
//	gTcpDataMark = 0;
	gLwipDataTime = 0;
}
// void Set_TcpMark(uint32_t value)
// {
// 	gTcpDataMark = value;
// }

void Set_LwipTimeover(uint32_t value)
{
	gLwipDataTime = value;
}

uint8_t GetLwip_Timemark(void)
{
	return gLwipDataTime;
}
// uint32_t Get_TcpDataMark(void)
// {
// 	return gTcpDataMark;
// }

// void set_ethreset_datamark(uint32_t value)
// {
// 	gEthResetMark = value;
// }
// uint32_t get_eth_datamark(void)
// {
// 	return gEthResetMark;
// }
...
*                                        ETH_DeInit
*
* @Description : De-initializes the ETHERNET peripheral registers to their default reset values.
* @Arguments   : None
* @Returns     : None
**********************************************************************************************************/
...
* @Arguments   : ETH_InitStruct: pointer to a ETH_InitTypeDef structure which will be initialized.
* @Returns     : None
**********************************************************************************************************/
void ETH_StructInit(ETH_InitTypeDef* ETH_InitStruct)
{
	...
	/*------------------------   MAC Configuration   ---------------------------*/
	ETH_InitStruct->ETH_AutoNegotiation             = ETH_AutoNegotiation_Disable;//--bt ETH_AutoNegotiation_Enable;			    /* PHY Auto-negotiation enabled                                         */
	ETH_InitStruct->ETH_Watchdog                    = ETH_Watchdog_Enable;					    /* MAC watchdog enabled: cuts off long frame                            */
	ETH_InitStruct->ETH_Jabber                      = ETH_Jabber_Enable;					    /* MAC Jabber enabled in Half-duplex mode                               */
	ETH_InitStruct->ETH_InterFrameGap               = ETH_InterFrameGap_96Bit;				    /* Ethernet interframe gap set to 96 bits                               */
	ETH_InitStruct->ETH_CarrierSense                = ETH_CarrierSense_Enable;				    /* Carrier Sense Enabled in Half-Duplex mode                            */
	ETH_InitStruct->ETH_Speed                       = ETH_Speed_100M; 						    /* PHY speed configured to 100Mbit/s                                    */
	ETH_InitStruct->ETH_ReceiveOwn                  = ETH_ReceiveOwn_Enable;			 	    /* Receive own Frames in Half-Duplex mode enabled                       */
	ETH_InitStruct->ETH_LoopbackMode                = ETH_LoopbackMode_Disable;			        /* MAC MII loopback disabled                                            */ 
	ETH_InitStruct->ETH_Mode                        = ETH_Mode_FullDuplex;					    /* Full-Duplex mode selected                                            */
	ETH_InitStruct->ETH_ChecksumOffload             = ETH_ChecksumOffload_Disable;			    /* IPv4 and TCP/UDP/ICMP frame Checksum Offload disabled                */
	ETH_InitStruct->ETH_RetryTransmission           = ETH_RetryTransmission_Enable;		        /* Retry Transmission enabled for half-duplex mode                      */ 
	ETH_InitStruct->ETH_AutomaticPadCRCStrip        = ETH_AutomaticPadCRCStrip_Disable;	        /* Automatic PAD/CRC strip disabled                                     */
	ETH_InitStruct->ETH_BackOffLimit                = ETH_BackOffLimit_10;					    /* half-duplex mode retransmission Backoff time_limit = 10 slot times   */ 
	ETH_InitStruct->ETH_DeferralCheck               = ETH_DeferralCheck_Disable;			    /* half-duplex mode Deferral check disabled                             */
	ETH_InitStruct->ETH_ReceiveAll                  = ETH_ReceiveAll_Disable;				    /* Receive all frames disabled                                          */
	ETH_InitStruct->ETH_SourceAddrFilter            = ETH_SourceAddrFilter_Disable;		        /* Source address filtering (on the optional MAC addresses) disabled    */
	ETH_InitStruct->ETH_PassControlFrames           = ETH_PassControlFrames_BlockAll;		    /* Do not forward control frames that do not pass the address filtering */
	ETH_InitStruct->ETH_BroadcastFramesReception    = ETH_BroadcastFramesReception_Disable;     /* Disable reception of Broadcast frames                                */
	ETH_InitStruct->ETH_DestinationAddrFilter       = ETH_DestinationAddrFilter_Normal;	        /* Normal Destination address filtering (not reverse addressing)        */
	ETH_InitStruct->ETH_PromiscuousMode             = ETH_PromiscuousMode_Disable; 		        /* Promiscuous address filtering mode disabled                          */
	ETH_InitStruct->ETH_MulticastFramesFilter       = ETH_MulticastFramesFilter_Perfect; 	    /* Perfect address filtering for multicast addresses                    */
	ETH_InitStruct->ETH_UnicastFramesFilter         = ETH_UnicastFramesFilter_Perfect; 	        /* Perfect address filtering for unicast addresses                      */
	ETH_InitStruct->ETH_HashTableHigh               = 0x0;									    /* Initialize hash table high and low regs                              */
	ETH_InitStruct->ETH_HashTableLow                = 0x0;
	ETH_InitStruct->ETH_PauseTime                   = 0x0;									    /* Flow control config (flow control disabled)                          */
	ETH_InitStruct->ETH_ZeroQuantaPause             = ETH_ZeroQuantaPause_Disable;
	ETH_InitStruct->ETH_PauseLowThreshold           = ETH_PauseLowThreshold_Minus4;
	ETH_InitStruct->ETH_UnicastPauseFrameDetect     = ETH_UnicastPauseFrameDetect_Disable;
	ETH_InitStruct->ETH_ReceiveFlowControl          = ETH_ReceiveFlowControl_Disable;
	ETH_InitStruct->ETH_TransmitFlowControl         = ETH_TransmitFlowControl_Disable;
	ETH_InitStruct->ETH_VLANTagComparison           = ETH_VLANTagComparison_16Bit;   //ETH_VLANTagComparison_16Bit; 		        /* VLANtag config (VLAN field not checked)      --bt                        */
	ETH_InitStruct->ETH_VLANTagIdentifier           = 0x0;

	/*---------------------- DMA Configuration   -------------------------------*/
	ETH_InitStruct->ETH_DropTCPIPChecksumErrorFrame = ETH_DropTCPIPChecksumErrorFrame_Disable;	/* Drops frames with with TCP/IP checksum errors                */
	ETH_InitStruct->ETH_ReceiveStoreForward         = ETH_ReceiveStoreForward_Enable; 			/* Store and forward mode enabled for receive                   */  
	ETH_InitStruct->ETH_FlushReceivedFrame          = ETH_FlushReceivedFrame_Enable; 			/* Flush received frame that created FIFO overflow              */  
	ETH_InitStruct->ETH_TransmitStoreForward        = ETH_TransmitStoreForward_Enable;   		/* Store and forward mode enabled for transmit                  */  
	ETH_InitStruct->ETH_TransmitThresholdControl    = ETH_TransmitThresholdControl_64Bytes;		/* Threshold TXFIFO level set to 64 bytes (used when threshold mode is enabled) */  
	ETH_InitStruct->ETH_ForwardErrorFrames          = ETH_ForwardErrorFrames_Disable;			/* Disable forwarding frames with errors (short frames, CRC,...) */  
	ETH_InitStruct->ETH_ForwardUndersizedGoodFrames = ETH_ForwardUndersizedGoodFrames_Disable;	/* Disable undersized good frames                                */  
	ETH_InitStruct->ETH_ReceiveThresholdControl     = ETH_ReceiveThresholdControl_64Bytes; 		/* Threshold RXFIFO level set to 64 bytes (used when Cut through mode is enabled) */
	ETH_InitStruct->ETH_SecondFrameOperate          = ETH_SecondFrameOperate_Disable; 			/* Disable Operate on second frame (transmit a second frame to FIFO without waiting status of previous frame */
	ETH_InitStruct->ETH_AddressAlignedBeats         = ETH_AddressAlignedBeats_Enable;			/* DMA works on 32-bit aligned start source and destinations addresses */  
	ETH_InitStruct->ETH_FixedBurst                  = ETH_FixedBurst_Enable;					/* Enabled Fixed Burst Mode (mix of INC4, INC8, INC16 and SINGLE DMA transactions */  
	ETH_InitStruct->ETH_RxDMABurstLength            = ETH_RxDMABurstLength_32Beat;				/* DMA transfer max burst length = 32 beats = 32 x 32bits */
	ETH_InitStruct->ETH_TxDMABurstLength            = ETH_TxDMABurstLength_32Beat;  
	ETH_InitStruct->ETH_DescriptorSkipLength        = 0x0; 										/* DMA Ring mode skip length = 0 */  
	ETH_InitStruct->ETH_DMAArbitration              = ETH_DMAArbitration_RoundRobin_RxTx_1_1;	/* Equal priority (round-robin) between transmit and receive DMA engines */
}
...
*                                        ETH_Init
*
* @Description : Initializes the ETHERNET peripheral according to the specified parameters 
*                in the ETH_InitStruct.
* @Arguments   : ETH_InitStruct: pointer to a ETH_InitTypeDef structure that contains the configuration 
*                                information for the specified ETHERNET peripheral.
*                PHYAddress: external PHY address
* @Returns     : ETH_ERROR: Ethernet initialization failed
...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
	...
	{
		tmpreg |= (uint32_t)ETH_MACMIIAR_CR_Div16;	/* CSR Clock Range between 20-35 MHz */
	}
	else if((hclk >= 35000000)&&(hclk < 60000000))
	{
		tmpreg |= (uint32_t)ETH_MACMIIAR_CR_Div26;	/* CSR Clock Range between 35-60 MHz */ 
	}  
	else if((hclk >= 60000000)&&(hclk < 100000000))
	{ 
		tmpreg |= (uint32_t)ETH_MACMIIAR_CR_Div42;	/* CSR Clock Range between 60-100 MHz */ 
	}  
	else if((hclk >= 100000000)&&(hclk < 150000000))
	{ 
		tmpreg |= (uint32_t)ETH_MACMIIAR_CR_Div62;	/* CSR Clock Range between 100-150 MHz */ 
	}
	else /* ((hclk >= 150000000)&&(hclk <= 168000000)) */
	{
		tmpreg |= (uint32_t)ETH_MACMIIAR_CR_Div102;	/* CSR Clock Range between 150-168 MHz */     
	}
...
error:
	...
	/* Wait until the write operation will be taken into account: at least four TX_CLK/RX_CLK clock cycles */
	tmpreg = ETH->MACCR;
...
*                ETH_SUCCESS:Ethernet successfully initialized
**********************************************************************************************************/
uint32_t reETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
	uint32_t RegValue = 0, tmpreg = 0;
	__IO uint32_t i   = 0;
	RCC_ClocksTypeDef  rcc_clocks;
	...
	if (ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable)
	{
		...
		while (!(ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_Linked_Status) && (timeout < 2000))
		{
			timeout += 50;
		...
		/* Wait until the auto-negotiation will be completed */
		while (!(ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_AutoNego_Complete) && (timeout < 8000))
		{
			timeout += 50;
			PLATFORM_OS_TimeDly(50);
		}
...
{ 
	...
	while (descriptor_scan_counter < ETH_RXBUFNB)
	{
		...
		if ((DMARxDescToGet->Status  & ETH_DMARxDesc_OWN) == (uint32_t)RESET)
		{
				...
				((DMARxDescToGet->Status&ETH_DMARxDesc_FS) != (uint32_t)RESET ))
			{
				...
				frame.descriptor = DMARxDescToGet; //DMA_RX_FRAME_infos->FS_Rx_Desc;
	...
	while (((DMARxDescToGet->Status  & ETH_DMARxDesc_OWN) == (uint32_t)RESET) && (descriptor_scan_counter < ETH_RXBUFNB))
	{
		/* Just by security */
		descriptor_scan_counter++;
		...
		{ 
			...
			/* Get the address of the buffer start address Check if more than one segment in the frame */
			if (DMA_RX_FRAME_infos->Seg_Count >1)
			{
				frame.buffer =(DMA_RX_FRAME_infos->FS_Rx_Desc)->Buffer1Addr;
			}
...
/**
  ...
  * @retval error status
  */
uint32_t ETH_Prepare_Transmit_Descriptors(u16 FrameLength,u8 sel)
{   
	...
	if(sel)
		DMATxDesc->Status            |= (ETH_DMATxDesc_FS|ETH_DMATxDesc_LS|ETH_DMATxDesc_VF);	/* set LAST and FIRST segment            */	
  else
    DMATxDesc->Status            |= (ETH_DMATxDesc_FS|ETH_DMATxDesc_LS);	/* set LAST and FIRST segment            */			
	DMATxDesc->ControlBufferSize  = (FrameLength & ETH_DMATxDesc_TBS1);     /* set frame size                        */		
	DMATxDesc->Status            |= ETH_DMATxDesc_OWN;                      /* gives the buffer back to ETHERNET DMA */
	...
	else
	{
		...
		{			
			DMATxDesc->Status &= ~(ETH_DMATxDesc_FS | ETH_DMATxDesc_LS); /* Clear FIRST and LAST segment bits */

			if (i == 0) 
			{				
				DMATxDesc->Status |= ETH_DMATxDesc_FS;  /* Setting the first segment bit */
			}
			...
			if (i == (buf_count-1))
			{				
				DMATxDesc->Status |= ETH_DMATxDesc_LS; /* Setting the last segment bit */
...
void ETH_DMATxDescChainInit(ETH_DMADESCTypeDef *DMATxDescTab, uint8_t* TxBuff, uint32_t TxBuffCount)
{
	...
	DMATxDescToSet = DMATxDescTab;	/* Set the DMATxDescToSet pointer with the first one of the DMATxDescTab list */
...
/**
  ...
  */
uint32_t ETH_GetRxPktSize(void) //(ETH_DMADESCTypeDef *DMARxDesc)
{
	uint32_t frameLength = 0;
	ETH_DMADESCTypeDef *DMARxDesc;
	
	DMARxDesc = (ETH_DMADESCTypeDef *)DMARxDescToGet;
