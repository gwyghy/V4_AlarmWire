...
uint16_t g_LcdHeight = 320;			/* 显示屏分辨率-高度 */
...
uint8_t ReadLCDID(void)
{
	...
	bsp_DelayMS(20);
	if((GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_8) ==0) && (GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_9) ==0))
	{
		type = 0;//tianma
	}
	else
...
static void LCD_FSMCConfig(void)
{
	...
	timingWrite.FMC_AddressSetupTime = 0x0F;
	timingWrite.FMC_AddressHoldTime = 0x00;
	timingWrite.FMC_DataSetupTime = 0x46;
	...
	timingRead.FMC_AddressSetupTime = 0x0F;
	timingRead.FMC_AddressHoldTime = 0x00;
	timingRead.FMC_DataSetupTime = 0x46;
...
void LCD_SetPwmBackLight(uint8_t _bright)
{
	...
	bsp_SetTIMOutPWM(GPIOA, GPIO_Pin_10, TIM10, 1, 100, (_bright * 10000) /255);
	#else
