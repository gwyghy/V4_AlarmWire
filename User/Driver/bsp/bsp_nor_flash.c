...
#define ADDR_SHIFT(A) 	(NOR_FLASH_ADDR + (4 * (A)))
#define NOR_SHIFT(A) 	(NOR_FLASH_ADDR + (2 * (A)))
...
void bsp_InitNorFlash(void)
{
	...
	/* NOR Flash 的 GPIO ：
		...
		PD6/FMC_NWAIT		- 忙信号，配置为GPIO，输入模式，通过软件查询方式判忙
...
//	GPIO_PinAFConfig(GPIOG, GPIO_PinSource12 , GPIO_AF_FMC);/**/
	...
	/*-- FMC Configuration ------------------------------------------------------*/
	p.FMC_AddressSetupTime = 0x04;			/* 0x05正常， 0x04 出错 */
	p.FMC_AddressHoldTime = 0x00;
	p.FMC_DataSetupTime = 0x06;			/* 0x0B正常， 0x0A 出错 */ 
...
//	NOR_WRITE(NOR_SHIFT(_uiWriteAddr), _usData);
