...
*              特别注意，这个程序是由M3移植到M4上面的，这里只是修改了USART1
*	修改记录 :
*		版本号  日期       作者    说明
...
/* 定义每个串口结构体变量 */
	...
	static uint8_t g_TxBuf4[UART1_TX_BUF_SIZE];		/* 发送缓冲区 */
	static uint8_t g_RxBuf4[UART1_RX_BUF_SIZE];		/* 接收缓冲区 */
#endif

#if UART5_FIFO_EN == 1
	static UART_T g_tUart5;
	static uint8_t g_TxBuf5[UART1_TX_BUF_SIZE];		/* 发送缓冲区 */
	static uint8_t g_RxBuf5[UART1_RX_BUF_SIZE];		/* 接收缓冲区 */
#endif
