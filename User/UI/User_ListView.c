...
extern uint8_t Super_advanced_password;
extern uint8_t  SinglePointnumber[];
extern uint8_t  SensorFloatPointnumber[];
extern uint8_t  SensorSelPointnumber[];
/*
*********************************************************************************************************
*	                                     宏定义
*********************************************************************************************************
...
uint8_t GetListviewData(uint64_t id)
{
	...
	{	
		...
		if((temp_value.Show_Noshow == DISPLAY_SHOW) ||(temp_value.Show_Noshow == DISPLAY_SUPER_SHOW))
		{
...
//	LISTVIEW_SetFont(hItem, &GUI_FontHZ32x33);
	for(i=0;i<count;i++)
	{
		...
		else if(MenuList[i].Parameter->Input_Type == EDIT_SIGNED_FLOAT) //ghy增加有符号浮点型数据
		{
			sprintf(buf, "%d.%d",MenuList[i].Para_Value/10, MenuList[i].Para_Value%10);	
			AddUnitDispiayINITLIST(buf,&MenuList[i]);			
			p[2] = buf;
...
//	LISTVIEW_SetFont(hItem, &GUI_FontHZ32x33);
	for(i=0;i<count;i++)
	{
		...
		if((Super_advanced_password == 0) && (MenuList[i].Parameter->Show_Noshow == DISPLAY_SUPER_SHOW))
		{
			continue;
		}
		...
		p[1] = MenuList[i].Parameter->Para_Name;	
		if((MenuList[i].Parameter->Input_Type == EDIT_SEL)
			||(MenuList[i].Parameter->Input_Type == EDIT_FLOAT_CHUANGANQI)
			...
			||((MenuList[i].Parameter->Input_Type > 99)))
		{
			...
			||(MenuList[i].Parameter->Input_Type == EDIT_SEL_CHUANGANQI))
		{	
			sprintf(buf, "%d", MenuList[i].Para_Value);
			AddUnitDispiayINITLIST(buf,&MenuList[i]);
			p[2] = buf;
		}
		else if(MenuList[i].Parameter->Input_Type == EDIT_INT_DANDONGZUO)
		{	
			sprintf(buf, "%d", MenuList[i].Para_Value);
			AddUnitDispiayINITLIST(buf,&MenuList[i]);
			p[2] = buf;
		...
		else if(MenuList[i].Parameter->Input_Type == EDIT_SIGNED_FLOAT) //ghy增加
		{
			sprintf(buf, "%d.%d",MenuList[i].Para_Value/10, MenuList[i].Para_Value%10);	
			AddUnitDispiayINITLIST(buf,&MenuList[i]);			
			p[2] = buf;
...
//	const char* decimal = {"."};
	...
	if(val == 0)
	{
		...
		if(MenuListtemp.Parameter.Input_Type == EDIT_SIGNED_INT \
			||MenuListtemp.Parameter.Input_Type == EDIT_SIGNED_FLOAT)
...
*********************************************************************************************************
*/
void LightingLedAccordingToInputType(uint8_t val)
{
	if(val == 0)
	{
		SendLEDNavigationData(0,KEY_ALL);
	}
	else if((val == 1) || (val == 6) || (val == 7)|| (val == 11)|| (val == 12) || (val > 99))
	{
		SendLEDNavigationData(KEY_CHOOSE,KEY_ALL);
	}
	else if(val == 10)
	{
	...
	{
		SendLEDNavigationData(KEY_NUMBER,KEY_ALL);
	}
}
/*
*********************************************************************************************************
...
*********************************************************************************************************
*/
void LightingLedAccordingToModiftType(uint8_t val)
{
	if(val == 3)
	{
		SendLEDNavigationData(KEY_LOCAL,0);
	}
	else
	{
		SendLEDNavigationData(KEY_OVERALL,0);
	}
}
/*
*********************************************************************************************************
...
void USER_EnterListviewRow(WM_MESSAGE * pMsg,uint8_t val)
{
	...
	hItem = WM_GetDialogItem(pMsg->hWin, GUI_ID_LISTVIEW0);
	Row = GetRowValue();
	if( MenuList[Row].Parameter ->Show_Noshow == 1)
	{
		return;
	}
	if( MenuList[Row].Parameter ->Pass_Word == 3)
	{
		if(Super_advanced_password != 1)
		{
			return;
		}
	}
	...
	LightingLedAccordingToInputType(MenuListtemp.Parameter.Input_Type);
	LightingLedAccordingToModiftType(MenuListtemp.Parameter.Modift_Type);
}
/*
*********************************************************************************************************
*	函 数 名: 
...
void USER_ExitListviewRow(WM_MESSAGE * pMsg)
{
	...
	LISTVIEW_SetItemBkColor(hItem, (uint8_t)2, (uint8_t)Row, LISTVIEW_CI_SELFOCUS, GUI_WHITE);
	LightingLedAccordingToInputType(0);
}
...
void USER_EditSelSetValue(WM_MESSAGE * pMsg,uint16_t key)
{
	...
	if(MenuListtemp.Parameter.Input_Type == EDIT_FLOAT_CHUANGANQI)
	{
		for(i=0;i<20;i++)
		{
			if(SensorFloatPointnumber[i] == 1)
			{
				//获取单动作的点号
				GUIGetDateFunAPI(API_Sensor_Version,i,&Sensortype,0);
				if(Sensortype.pointID.point == MenuListtemp.Parameter.Para_Value)
				{
...
//				MenuListtemp.Parameter.Para_Value = MenuListtemp.Parameter.Min_Value;
	...
	if(MenuListtemp.Parameter.Input_Type == EDIT_SEL_CHUANGANQI)
	{
		for(i=0;i<20;i++)
		{
			if(SensorSelPointnumber[i] == 1)
			{
				//获取单动作的点号
				GUIGetDateFunAPI(API_Sensor_Version,i,&Sensortype,0);
				if(Sensortype.pointID.subDevice == MenuListtemp.Parameter.Para_Value)
				{
					//MenuListtemp.NumericRange = 1;
					MenuListtemp.NumericRange = 0; //ghy 设置子设备号时可以
					break;
				}
			}
		}
...
*********************************************************************************************************
*/
void USER_EditIntSingleSetValue(WM_MESSAGE * pMsg,uint16_t key)
{
	WM_HWIN hItem;
	uint8_t row;
	uint8_t i;
	SigActionParamType Actiontype;
	char * p;
	char buf[15];
	hItem = WM_GetDialogItem(pMsg->hWin, GUI_ID_LISTVIEW0);
	row = GetRowValue();
	...
	{ 
		case USER_KEY_UP:
			...
			for(i=0;i<50;i++)
			{
				if(SinglePointnumber[i] == 1)
				{
					//获取单动作的点号
					GUIGetDateFunAPI(API_Singel_Action,i,&Actiontype,0);
					if(Actiontype.baseParam.pointID.point == MenuListtemp.Parameter.Para_Value)
					{
						MenuListtemp.NumericRange = 1;
						break;
					}
		...
		case USER_KEY_DOWN:
			...
			for(i=0;i<50;i++)
			{
				if(SinglePointnumber[i] == 1)
				{
					//获取单动作的点号
					GUIGetDateFunAPI(API_Singel_Action,i,&Actiontype,0);
					if(Actiontype.baseParam.pointID.point ==0)
						break;
					if(Actiontype.baseParam.pointID.point == MenuListtemp.Parameter.Para_Value)
					{
						MenuListtemp.NumericRange = 1;
...
void USER_EditSignedIntSetValue(WM_MESSAGE * pMsg,uint16_t key)
{
	...
	{ 
		...
		case USER_KEY_LEFT3:
			...
			{
//				MenuListtemp.Parameter.Para_Value = MenuListtemp.Parameter.Min_Value;
				if((int16_t)MenuListtemp.Parameter.Min_Value < 0)
				{
					if((int16_t)MenuListtemp.Parameter.Para_Value < 0)
					{
						MenuListtemp.Parameter.Para_Value = MenuListtemp.Parameter.Min_Value;
					}
				}
...
*********************************************************************************************************
*/
void USER_EditSignedFloatSetValue(WM_MESSAGE * pMsg,uint16_t key)
{
	WM_HWIN hItem;
	uint8_t row;
	static BOOL sing = 0; //正负号标志
...
//				MenuListtemp.Parameter.Para_Value = MenuListtemp.Parameter.Min_Value;
				...
				MenuListtemp.NumericRange = 0;
			//sprintf(buf, "%d",(int16_t)MenuListtemp.Parameter.Para_Value);		
			sprintf(buf, "%d.%d",(int16_t)MenuListtemp.Parameter.Para_Value/10,MenuListtemp.Parameter.Para_Value%10);
			AddUnitDispiaySETLIST(buf,&MenuListtemp);
			p = buf;
			LISTVIEW_SetItemText(hItem, 2, row, p);
			break;	
...
{	
	...
	{ 
		...
		case EDIT_SEL_CMD:
			...
			break;
		case EDIT_INT:
		case EDIT_SEL_CHUANGANQI:
		    USER_EditIntSetValue(pMsg,key);
			break;
		case EDIT_INT_DANDONGZUO:
		    USER_EditIntSingleSetValue(pMsg,key);
			break;
//		case EDIT_INT_CHUANGANQI:
//		    USER_EditIntchuanganqiSetValue(pMsg,key);
//			break;
		case EDIT_FLOAT:
		...
		case EDIT_SIGNED_INT:
			...
			break;
		case EDIT_SIGNED_FLOAT: //有符号浮点数处理
			USER_EditSignedFloatSetValue(pMsg,key);
			break;
		default:
			break;
	}
