...
extern uint16_t targer,targerold;	
extern WM_HWIN GroupLeft_hWin,MasterContor_hWin,PassWord_hWin,RackContor_hWin;
/*
*********************************************************************************************************
*	函 数 名: 
*	功能说明: 设置当前页数
...
*********************************************************************************************************
*/
void ChangeInfoFollow(WM_HWIN hWin,uint16_t  pStr)
{ 
	 WM_MESSAGE Message;
	 Message.MsgId = MSG_CHANGE_FOLLOW;
	 Message.Data.v = pStr;
	 WM_SendMessage(hWin, &Message);
}
/*
...
//	uint16_t time = 500;
	//获取超时时间
	GUIGetDateFunAPI(API_Current_Timr,0,&time,&length);
	if(overtime >= 0)
		overtime++;
	if(overtime > (time*100/15))
	{
		GUI_EndDialog(pMsg->hWin, 0);
		if((CurrentWindow == followwin) || (CurrentWindow == groupwin) || (CurrentWindow == masterwin) || (CurrentWindow == passwin)|| (CurrentWindow == rackwin)|| (CurrentWindow == slverwin))
		{
			if(CurrentWindow == masterwin)
			{
				alar[0] = 1;
				alar[1] = targer;				
				SysDataFun(DATA_MSG_CTRL_ALARM,0,0,2,0,&alar[0],0);	
			}
			CreateHomeFrame();	
		}
		else
...
void Setstheinitialstate(void)
{
	...
	if((temp.emergencyState == 1) || (temp.bisuoState == 1) ||(temp.stopState == 1) || (temp.hwEmergency == 1) || (temp.hwBisuo == 1) ||(temp.hwStop == 1))
	{
...
{	
	...
	if(!Initialstate)//原来不是急停闭锁状态
	{
		if((temp.hwEmergency == 1) || (temp.hwBisuo == 1) ||(temp.hwStop == 1))
		{
			back = 1;
		}	
	}
