...
static void HomeFrame_cbDialog(WM_MESSAGE * pMsg) 
{
	...
	static uint8_t timer,timer1,sever;
			...
			switch(getFileRecvStatus())
			{
				case FILEDATA:
					if(frist == 0)
					{
						...
						TEXT_SetFont(WM_GetDialogItem(pMsg->hWin, GUI_ID_TEXT22), &GUI_FontHZ30x30);
					}	
					memset(buf,0,20);
						...
						else
						{
							if(timer > 4)
							{
								...
								if(!SupportStatusdata)
								{
									...
									TEXT_SetText(WM_GetDialogItem(pMsg->hWin, GUI_ID_TEXT24),"连接服务器失败");
								}
							}
						}
						goto loop;	
			...
			if(sever)
			{
				...
				else
				{
					if(timer > 14)
					{
						...
						pass = 0;
						count = 0;
						TEXT_SetText(WM_GetDialogItem(pMsg->hWin, GUI_ID_TEXT30), "连接服务器失败");
					}
				}
			}
...
WM_HWIN CreateHomeFrame(void) 
{
	...
	{
		CreateMoveFrame_RightTemp();
	}
	else
	{
		CreateMoveFrame_LeftTemp();
	}
